...
def sha256(p):
 ...
 return h.hexdigest()
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--repo',default='.'); a=ap.parse_args(); r=Path(a.repo).resolve(); checks={}
 checks['canonical_archive_sha256']=sha256(r/'canonical_archive/IGAR-Paper1-Reproducibility_STRICT_FINAL.zip')==CANON_SHA
 checks['p1r11_zip_sha256']=sha256(r/'revision_addenda/IGAR_Paper1_P1R11_Reproducibility_Addendum_v0.6_CLOSED.zip')==ADD_SHA
 ...
 checks['root_status_p1r11_closed']=st.get('revision_stage_addendum',{}).get('status')=='CLOSED'
 ...
 # verify internal SHA manifest
 bad=[]
 for line in (p/'manifests/SHA256SUMS.txt').read_text(encoding='utf-8').splitlines():
  if not line.strip(): continue
  h,rel=line.split(None,1); rel=rel.strip(); fp=p/rel
  if not fp.exists() or sha256(fp)!=h: bad.append(rel)
 checks['internal_52_checksum_manifest']=len(bad)==0
 # verify repo manifest aliases equal and all entries
 m1=(r/'MANIFEST_PUBLIC_RELEASE_v1.1.0.sha256').read_bytes(); ma=(r/'MANIFEST_PUBLIC_RELEASE.sha256').read_bytes()
 checks['repo_manifest_alias_matches']=m1==ma
 rbad=[]
 for line in m1.decode('utf-8').splitlines():
  if not line.strip(): continue
  h,rel=line.split(None,1); fp=r/rel.strip()
  if not fp.exists() or sha256(fp)!=h: rbad.append(rel.strip())
 checks['repo_manifest_entries_verify']=len(rbad)==0
 out={'all_pass':all(checks.values()),'checks':checks,'internal_bad_files':bad,'repo_bad_files':rbad}
